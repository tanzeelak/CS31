...
private:
    ...
    int beenHit = 0;
    //each robot is an element in the array. element is changed to 1, hit
};
...
void Robot::move()
{
    ...
    switch (dir)
    {
        case 2:
            ...
            break;
        case 3:
            if (m_col+1 < m_arena->cols()) //we are looking at pointer to m_arena, whch is a pointer to Arena ap, to access the member cols()
                m_col++;
            break;
        case 0:
            if (m_row-1 > 0)
...
bool Robot::getAttacked(int dir)  // return true if dies
{
    ...
    if (beenHit == 0)
    {
        ...
        {
            if (m_col+1 <= m_arena->cols()) //check if no wall
                m_col++;
            else
                return true;
        }
    ...
    return true; //dir ==1 (has been hit before). so die
...
void Player::moveOrAttack(int dir)
{
    ...
    if (dir == 2)
    {
        if (m_arena->nRobotsAt(m_row, m_col -1) > 0) //if any robots there
        {
            m_arena->attackRobotAt(m_row, m_col-1, 2);
            //m_arena->attackRobotAt(m_row, m_col-1, 0); //DELETE LATER
        }
        else if (m_col-1 > 0) //if no wall
            m_col--;//move player in that direction
    } //there is a wall, so don't move at all
    if (dir == 3)
    {
        if (m_arena->nRobotsAt(m_row, m_col +1) > 0)
        {
            m_arena->attackRobotAt(m_row, m_col+1, 3);
            //m_arena->attackRobotAt(m_row, m_col+1, 0);
        }
    ...
    if (dir == 0)
    {
        ...
        {
            m_arena->attackRobotAt(m_row-1, m_col, 0);
            //m_arena->attackRobotAt(m_row-1, m_col, 0);
        }
        else if (m_row-1 > 0)
            m_row--;//grid(r-1, c) = '@'
    ...
    if (dir == 1)
    {
        if (m_arena->nRobotsAt(m_row + 1, m_col) > 0)
        {
            m_arena->attackRobotAt(m_row+1, m_col, 1);
            //m_arena->attackRobotAt(m_row+1, m_col, 0);
...
Arena::Arena(int nRows, int nCols)
{
    ...
    m_rows = nRows;
    m_cols = nCols;
    m_player = nullptr; //initially equals nullptr
    m_nRobots = 0;
}
...
Arena::~Arena()
{
    ...
    if (player() != nullptr) //FOCUS: if still exist
        delete player();
...
int Arena::nRobotsAt(int r, int c) const
{
    ...
    int robotsCounter = 0;
    for (int i = 0; i < m_nRobots; i++) //running through all robots
    {
        if (m_robots[i]->row() == r && m_robots[i]->col() == c) //FOCUS. HOW AM I ABLE TO ACCESS m_robots? Isn't row() a constant
            robotsCounter++;
    }
...
void Arena::display() const
{
    ...
    for (int tempr = 0; tempr < rows(); tempr++){
        for (int tempc = 0; tempc < cols(); tempc++){
            int x = nRobotsAt(tempr+1, tempc+1); //FOCUS: why "+1" //because arrays start at 0
            if (x > 0) //if there are robots
            {
                switch (nRobotsAt(tempr+1, tempc+1))
                {
                    case 1:
                        grid[tempr][tempc] = 'R';
                        break;
...
bool Arena::addRobot(int r, int c)
{
    ...
    if (m_nRobots == MAXROBOTS)
        return false;
    m_robots[m_nRobots] = new Robot(this, r, c); //FOCUS: what does "this" refer to? REFERS TO ARENA'S this
    m_nRobots++;
...
bool Arena::attackRobotAt(int r, int c, int dir)
{
    ...
    int didItDie;
    for (int i = 0; i < m_nRobots; i++) //running through all robotes
    {
        if (m_robots[i]->row() == r && m_robots[i]->col() == c)//do until you find a robot at position
        {
            didItDie =  m_robots[i]->getAttacked(dir); //getAttacked returns true if died
            if (didItDie)
            {
                delete m_robots[i];
                m_robots[i] = m_robots[m_nRobots - 1]; //set the pointer to the last robot in the array since you won't access that anymore (m_nRobots is being decremented)
                                                        //you were doing m_robots[i] = m_robots[i+1] before which is wrong.
    ...
    return false;
    //if this is the second attack to robot
...
bool Arena::moveRobots()
{
    for (int k = 0; k < m_nRobots; k++)
    {
        ...
        m_robots[k]->move();//move one robot at a time
        if (m_robots[k]->row() == m_player->row() && m_robots[k]-> col() == m_player->col())//FOCUS: why -> and not dot operator
            m_player->setDead();
    }
